...
class ResizeModule:
    ...
    def run(self, width, height):
        ...
        try:
            # Create node
            with dpg.node(parent="MainNodeEditor", tag="Node_Resize_Image", label="Resize image", pos=[500, 20]):
                with dpg.node_attribute():
                    dpg.add_input_int(tag="x_size", label="x", width=100)
                    dpg.add_input_int(tag="y_size", label="y", width=100)
                    dpg.add_input_int(tag="z_size", label="z", width=100, default_value=self.img.size[0])
                    dpg.add_input_int(tag="w_size", label="w", width=100, default_value=self.img.size[1])
            ...
            with dpg.popup("Node_Resize_Image"):
                dpg.add_button(label="Delete", callback=lambda: dpg.delete_item("Node_Resize_Image"))
