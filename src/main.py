...
from src.NodesCore.features import ResizeModule, RotateModule
from src.NodesCore.display import ImageModule, OutputModule

IMAGE_PATH = r"src/example.png"
DISPLAY_TOOLTIPS: bool = False # Set to True to display tooltips
...
with dpg.window(
    ...
    with dpg.node_editor(tag="MainNodeEditor", callback=lambda sender, app_data: dpg.add_node_link(app_data[0], app_data[1], parent=sender),
...
#        dpg.add_button(label="Delete", callback=lambda: dpg.delete_item("Node_Output_Image"))
    ...
    if DISPLAY_TOOLTIPS: # Toolstips at the end so everything initialize before
        for module in modules:
