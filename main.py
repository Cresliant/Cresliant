...
from src.utils.view import auto_align
...
with dpg.window(
    ...
    with dpg.menu_bar():
        ...
        with dpg.menu(tag="help", label="Help"):
            ...
            dpg.add_separator()
            dpg.add_menu_item(
                tag="version", label="Cresliant " + (f"{VERSION} DEBUG" if node_editor.debug else VERSION)
            )

    dpg.add_text("For help with using Cresliant, press Ctrl+H.", bullet=True)
    node_editor.start()
    auto_align("manual_modal", 2)
...
if node_editor.debug:
    # This makes the debuggers work by actually stopping at breakpoints
